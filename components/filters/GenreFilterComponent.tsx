...
const GenreFilterComponent: NextComponentType<NextPageContext, {}, Props> = ({ instanceId, filterUrl }: Props) => {
	...
	const initValue = useCallback(async () => {
		...
		if (genresQueries) {
			const { results: genres } = await GenreService.getGenreList({
				id__in: Array.isArray(genresQueries) ? genresQueries.join(',') : genresQueries, size: 100
			});
