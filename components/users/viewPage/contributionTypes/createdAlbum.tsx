    ...
    <>
      ...
      <p>
        ...
        <Link href={`/album/${album.slug}`}>
          <a className="font-bold text-primary-accent hover:underline">{album.title}</a>
        </Link>
        {" by "}
        <Link href={album.artist__name ? `/artist/${album.artist__slug}` : `/band/${album.band__slug}`}>
          <a className="italic text-primary-accent hover:underline">{album.artist__name || album.band__name}</a>
        </Link>
      </p>
