    ...
    <>
      ...
      <p>
        ...
        {bandMember.artist__name ? (
          <Link href={`/artist/${bandMember.artist__slug}`}>
            <a className="font-bold text-primary-accent hover:underline">{bandMember.artist__name}</a>
          </Link>
        ) : (
          <span className="font-bold">{bandMember.name}</span>
        )}
        {` as a member to the band called `}
        <Link href={`/band/${bandMember.band__slug}`}>
          <a className="font-bold text-primary-accent hover:underline">{bandMember.band__name}</a>
        </Link>
      </p>
