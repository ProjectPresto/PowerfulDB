  ...
  return (
    <>
      ...
      <p>
        ...
        {` to the album called `}
        <Link href={`/album/${genres.album_genres__slug}`}>
          <a className="italic text-main-accent hover:underline">{genres.album_genres__title}</a>
        </Link>
      </p>
      <span className="text-gray-500 italic hidden md:inline">{`+ ${genres.genres_count} ${genres.genres_count === 1 ? " point" : " points"}`}</span>
    </>
  );
};
