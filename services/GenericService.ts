...
abstract class GenericClass {
			...
			Object.entries(urlQueries).forEach(([key, value], index) => {
				if (Array.isArray(value)) {
					value.forEach((param, paramIndex) => {
						if (paramIndex === 0 && index === 0) url += `?${key}=${param}`; else url += `&${key}=${param}`;
					});
				} else {
					if (index === 0) url += `?${key}=${value}`; else url += `&${key}=${value}`;
				}
			});
		}
	...
	public async postItem<T>(path: string, payload: object, config?: object) {
		const { data } = await HttpService.http.post<T>(`/${path}/`, payload, config);
		return data;
	}
}
